...
const DropdownIndicator = (props: DropdownIndicatorProps<Option>) => {
  const clName = clsx(
    'cursor-pointer lg:w-3 lg:h-5 rotate-0 transition-all duration-300 mr-2',
    {
...
const Select = ({
  ...
  const containerClName = clsx(
    'static p-0 lg:p-[9px] text-blue-400 border-1 border-[#838688] bg-[#f5f1f1] rounded-[4px]',
    {
    ...
    <div className="relative w-full max-w-[808px]">
      <ReactSelect
        ...
        classNames={{
          ...
          control: () =>
            'cursor-pointer text-base lg:text-[24px] pl-3 lg:pl-[33px] text-[#2f2d2d] leading-[18px] lg:leading-[29px] flex gap-[8px]',
          menu: () =>
            'top-0 w-full p-3 left-0 bg-[#F5F1F1] border-1 rounded-[4px] border-[#838688]',
      ...
      {errorMessage && (
        <p className="absolute -bottom-3 left-8 text-xs leading-0 text-[#b71010]">
          {errorMessage}
        </p>
      )}
    </div>
