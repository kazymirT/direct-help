      ...
      <div className="flex flex-col max-sm:items-center sm:flex-row gap-3 sm:gap-1.5 md:gap-2.5 lg:gap-3.5">
        {ATTAINMENT_DATA.map(({ description, id, title }) => (
          <article key={id} className={cardCN}>
            <h3 className="text-center text-[45px] sm:text-[clamp(1.875rem,-0.938rem+5.859vw,2.813rem)] lg:text-[clamp(2.813rem,-0.111rem+4.567vw,4rem)]">{id !== 3 ? title : t('moneyInfo', {amount: title})}</h3>
            <p className="text-center text-[18px] sm:text-[clamp(0.875rem,0.125rem+1.563vw,1.125rem)] lg:text-[clamp(1.125rem,0.202rem+1.442vw,1.5rem)]">{t(description)}</p>
           </article> 
        ))}
      </div>
