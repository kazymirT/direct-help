...
const SupportForm: FC<SupportFormProps> = ({ setStatus }) => {
  ...
  return (
    <section className="wrapper relative m-auto flex flex-col gap-5 pt-7 pb-10 md:pb-15 lg:gap-[63px] lg:pt-[62px]">
      <h1 className="text-[clamp(1.5rem,1.236rem+1.127vw,2.25rem)] leading-[1.167] font-medium text-[#2f2d2d]">
        {t('title')}
      </h1>
      <form
        onSubmit={handleSubmit(onSubmit)}
        className="flex w-full max-w-[1170px] flex-col gap-[25px] lg:gap-18"
      >
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Select
                  id="transmission"
                  options={[
                    t('transmission.variants.one'),
                    t('transmission.variants.two'),
                  ]}
